...
export const CustomAuthGuard: (type?: string | string[]) => Type<IAuthGuard> = memoize(createAuthGuard);
...
function createAuthGuard(): Type<CanActivate> {
    class MixinAuthGuard<TUser = any> implements CanActivate {
        ...
        async logIn<TRequest extends { logIn: (...args: any) => any } = any>(request: TRequest): Promise<void> {
            const user = request[this.options.property || defaultOptions.property];
            await new Promise((resolve, reject) =>
                request.logIn(user, (err) => (err ? reject(err) : resolve(undefined))),
            );
        }
...
const createPassportContext = (request, response) => (type, options, callback: (...args: any) => any) =>
